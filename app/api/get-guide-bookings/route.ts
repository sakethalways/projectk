...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get guide ID for this user - look in guides table
    ...
    // Fetch booking requests for this guide's itineraries
    ...
    console.log('Tourist IDs to fetch:', touristIds);
    
    if (touristIds.length > 0) {
      // First check if tourist profiles table has any data
      const { data: allProfiles } = await serviceSupabase
        .from('tourist_profiles')
        .select('id, user_id, name')
        .limit(5);
      
      console.log('Sample tourist profiles in DB:', allProfiles);
      ...
      if (profileError) {
        console.error('Error fetching tourist profiles:', profileError);
      } else {
    ...
    console.log('Final bookings with tourists:', bookingsWithTourists);
