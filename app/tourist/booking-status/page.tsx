      ...
      <main className="flex-1 w-full lg:w-0 px-4 sm:px-6 py-6 sm:py-10 max-w-5xl mx-auto">
        ...
        {bookings.length === 0 ? (
          ...
          <div className="space-y-8">
            ...
            {/* Past Bookings */}
            {bookings.filter(b => ['cancelled', 'completed', 'past'].includes(b.status)).length > 0 && (
              <div>
                <h2 className="text-2xl font-bold text-foreground mb-4">Past Bookings</h2>
                <div className="grid gap-4">
                  {bookings
                    .filter(b => ['cancelled', 'completed', 'past'].includes(b.status))
                    .map((booking) => (
                      <Card key={booking.id} className="border border-border p-6 opacity-75">
                        <div className="flex flex-col gap-4">
                          {/* Info */}
                          <div>
                            <div className="flex items-center gap-3 mb-3">
